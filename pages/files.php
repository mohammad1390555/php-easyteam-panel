...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fileAction === 'upload' && isset($_FILES['file'])) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            $targetFile = $fullPath . '/' . basename($file['name']);
    ...
    if ($fileAction === 'rename') {
        ...
        $newPath = $basePath . '/' . dirname($oldName) . '/' . basename($newName);
        
        if ($oldPath && strpos($oldPath, realpath($basePath)) === 0 && !empty($newName)) {
        ...
        <?php else: ?>
            ...
            <div class="file-item file-item-<?= $item['is_dir'] ? 'folder' : 'file' ?>">
                ...
                <span class="file-actions">
                    <button class="btn btn-xs btn-secondary" onclick="renameItem('<?= htmlspecialchars(addslashes($item['path'])) ?>', '<?= htmlspecialchars(addslashes($item['name'])) ?>')"><svg class="icon"><use href="assets/icons/sprite.svg#icon-edit"/></svg></button>
                    <button class="btn btn-xs btn-danger" onclick="deleteItem('<?= htmlspecialchars(addslashes($item['path'])) ?>')"><svg class="icon"><use href="assets/icons/sprite.svg#icon-trash"/></svg>️</button>
                </span>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
