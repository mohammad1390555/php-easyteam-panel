...
class ServerManager {
    ...
    public static function installVersion(string $version, string $type = 'paper'): array {
        ...
        if (!file_exists($jarPath) || filesize($jarPath) < 1000) {
            // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionunlink($jarPath);
    ...
    private static function doDownload(string $url, string $destination): bool {
        ...
        $content = // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionfile_get_contents($url, false, $context);
        if ($content !== false) {
        ...
        if (function_exists('curl_init')) {
            ...
            // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionunlink($destination);
        }
    ...
    public static function createServer(array $data): array {
        ...
        // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionchmod($serverDir . '/start.sh', 0755);
    ...
    public static function sendConsoleCommand(int $serverId, string $command): array {
        ...
        if (!file_exists($stdinFifo) && function_exists('posix_mkfifo')) {
            // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionposix_mkfifo($stdinFifo, 0666);
        }

        // Try writing to FIFO first (created during server start)
        if (file_exists($stdinFifo) && is_writable($stdinFifo)) {
            $fh = // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionfopen($stdinFifo, 'w');
            if ($fh) {
                fwrite($fh, $command . "\n");
                fclose($fh);
            }
        ...
        if ($pid && !self::isWindows() && self::isProcessRunning($pid)) {
            ...
            if ($returnCode !== 0) {
                ...
                // // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionchmod($cmdFile, 0666);
            }
        }
...
SCRIPT;
    ...
    private static function generateWindowsStartScript(int $ram): string {
        $template = <<<'SCRIPT'
// // // // // // // @error suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionerror suppressionecho off
