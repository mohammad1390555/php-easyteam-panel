...
class ServerManager {
     ...
     * Priority: Local storage/versions/ first, then download from API
     */
    ...
    /**
     * Check if a local version file exists in storage/versions/
     */
    public static function getLocalVersionPath(string $version, string $type = 'paper'): ?string {
        $versionsDir = MINECRAFT_VERSIONS_PATH;
        $jarName = $type === 'paper' ? "paper-{$version}.jar" : "vanilla-{$version}.jar";
        $jarPath = $versionsDir . '/' . $jarName;
        ...
        // Also try without type prefix for paper
        if ($type === 'paper') {
            $altPath = $versionsDir . "/paper-{$version}.jar";
            if (file_exists($altPath) && filesize($altPath) > 1000) {
                return $altPath;
            }
        }
    ...
    public static function createServer(array $data): array {
        ...
        // Check if version is installed locally (storage/versions/) first
        $localPath = self::getLocalVersionPath($version, $type);
        ...
        // If local file exists but not in DB, register it
        if ($localPath && !$versionInstalled) {
            Database::insert('server_versions', [
                'version' => $version,
                'type' => $type,
                'installed' => 1,
        ...
        $installResult = ['path' => ''];
        if (!$versionInstalled && !$localPath) {
            // Try to install the version via download
    ...
    /**
     * Get console output for a server (optimized - uses fseek for large files)
     */
    public static function getConsoleOutput(int $serverId, int $lines = 100): array {
        ...
        if ($fileSize < 1048576) { // < 1MB - use efficient tail reading
            ...
            $allLines = explode("\n", $buffer);
            $output = array_slice($allLines, max(0, count($allLines) - $lines));
            $output = array_map('trim', $output);
        } else {
            // For very large files, use shell tail command if available
            $cmd = 'tail -n ' . escapeshellarg($lines) . ' ' . escapeshellarg($logFile) . ' 2>/dev/null';
            exec($cmd, $output, $returnCode);
...
SCRIPT;
    ...
    /**
     * Get a setting value (cached in session for performance)
     */
    public static function getSetting(string $key, string $default = ''): string {
        // Check session cache first
        if (isset($_SESSION['_settings_cache'][$key])) {
        ...
        // Cache in session (max 100 keys)
        if (!isset($_SESSION['_settings_cache'])) {
            $_SESSION['_settings_cache'] = [];
        }
        if (count($_SESSION['_settings_cache']) < 100) {
    ...
    /**
     * Set a setting value (updates cache too)
     */
