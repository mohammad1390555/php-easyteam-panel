...
<head>
    ...
    <!-- Vazir Font (Persian) + Inter Font (English) - Local -->
    <style>
        @font-face {
            font-family: 'Vazir';
...
<body data-page="<?= htmlspecialchars($page ?? 'login') ?>" data-lang="<?= Language::getCurrentLanguage() ?>">
    <!-- SVG Sprite -->
    <?php 
    $spriteCache = __DIR__ . '/../storage/cache/sprite.cache';
    $spriteDir = dirname($spriteCache);
    if (!is_dir($spriteDir)) {
        @mkdir($spriteDir, 0755, true);
    }
    $spriteContent = '';
    if (file_exists($spriteCache) && filemtime($spriteCache) > filemtime(__DIR__ . '/../assets/icons/sprite.svg')) {
        $spriteContent = file_get_contents($spriteCache);
    } else {
        $spriteSource = @file_get_contents(__DIR__ . '/../assets/icons/sprite.svg');
        if ($spriteSource) {
            $spriteContent = $spriteSource;
            @file_put_contents($spriteCache, $spriteSource);
        }
    }
    ?>
    <?php if ($spriteContent): ?>
        <?= $spriteContent ?>
    <?php else: ?>
    ...
    <div class="app-container">
        ...
        <main class="main-content" id="mainContent">
            ...
            <header class="topbar">
                ...
                <div class="sidebar-backdrop" id="sidebarBackdrop" onclick="document.getElementById('sidebar')?.classList.remove('open')" style="display:none;"></div>
                ...
                <div class="topbar-right">
                    <button class="theme-toggle" id="themeToggle" title="تغییر تم" aria-label="Toggle theme">
                        <svg class="icon" id="themeIcon"><use href="assets/icons/sprite.svg#icon-paper"/></svg>
                    </button>
                    <div class="language-switcher" id="langSwitcher">
                        <a href="#" data-lang="fa" class="lang-btn <?= Language::getCurrentLanguage() === 'fa' ? 'active' : '' ?>" onclick="switchLanguage('fa'); return false;">فا</a>
                        <a href="#" data-lang="en" class="lang-btn <?= Language::getCurrentLanguage() === 'en' ? 'active' : '' ?>" onclick="switchLanguage('en'); return false;">EN</a>
                    </div>
